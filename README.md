...
context saves us from having to props drill so much by sharing state down a component tree. 

2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

actions are like what is being delivered when they are called. they contain instructions on what to do when conditions are met to call them. reducers take an action and previous state to make a new state. store keeps the whole state of the app in an immuteable object tree. 

3. What is the difference between Application state and Component state? When would be a good time to use one over the other?

application state is global where as component state is local to that component. applications state is better for sending state thoughout an application where as component state is only good if you only need that state rendered once in that component and nohwere else in the app. 

4. Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

Thunk lets you call actions that return a function that wait for async operations to be completed.

5. What is your favorite state management system you've learned and this sprint? Please explain why!

Context API simply because its less setup and more streamline. 
